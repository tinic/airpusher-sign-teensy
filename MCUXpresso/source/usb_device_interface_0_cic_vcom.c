...
extern usb_device_endpoint_struct_t g_UsbDeviceInterface1DicVcomSetting0UsbserialdicEndpoints[];
...
usb_status_t USB_DeviceInterface0CicVcomCallback(class_handle_t handle, uint32_t event, void *param)
{
    ...
    switch (event)
    {
        ...
        {
            if ((epCbParam->length != 0) && (!(epCbParam->length % g_UsbDeviceInterface1DicVcomSetting0UsbserialdicEndpoints[USB_DIC_VCOM_IN_ENDPOINT_INDEX].maxPacketSize)))
            {
            ...
            else if ((1 == s_UsbDeviceComposite->attach) && (1 == s_UsbInterface0CicVcom.startTransactions))
            {
                if ((epCbParam->buffer != NULL) || ((epCbParam->buffer == NULL) && (epCbParam->length == 0)))
                {
                    ...
                    error = USB_DeviceCdcAcmRecv(handle, USB_DIC_VCOM_OUT_ENDPOINT, s_currRecvBuf,
                                                 g_UsbDeviceInterface1DicVcomSetting0UsbserialdicEndpoints[USB_DIC_VCOM_OUT_ENDPOINT_INDEX].maxPacketSize);
#if defined(FSL_FEATURE_USB_KHCI_KEEP_ALIVE_ENABLED) && (FSL_FEATURE_USB_KHCI_KEEP_ALIVE_ENABLED > 0U) && \
    defined(USB_DEVICE_CONFIG_KEEP_ALIVE_MODE) && (USB_DEVICE_CONFIG_KEEP_ALIVE_MODE > 0U) &&             \
    defined(FSL_FEATURE_USB_KHCI_USB_RAM) && (FSL_FEATURE_USB_KHCI_USB_RAM > 0U)
